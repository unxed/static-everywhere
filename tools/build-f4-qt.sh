...
f4_gallery_patch_cleanup() {
    if [ "${_F4_GALLERY_PATCH_APPLIED}" -eq 1 ]; then
        git -C "${SRC}" apply --unidiff-zero --reverse --quiet "${GALLERY_PATCH}"
    fi
}
trap f4_gallery_patch_cleanup EXIT
plan_step "cd ${SRC} && if git apply --unidiff-zero --check \"${GALLERY_PATCH}\"; then git apply --unidiff-zero --quiet \"${GALLERY_PATCH}\"; _F4_GALLERY_PATCH_APPLIED=1; elif git apply --unidiff-zero --reverse --check \"${GALLERY_PATCH}\"; then :; else echo 'error: ZoinGallery does not match the software-image overlay' >&2; exit 1; fi"
