...
- **["Perfectly Portable Linux Binary"](https://kuukkanen.net/posts/portable-linux-binary/)**, Jarkko Kuukkanen (2024) — someone arriving at this project's core thesis independently: "there is no hard requirement for the application to use any libraries whatsoever" for anything reachable over the kernel's own syscall/socket interface, because everything on Linux that isn't the GPU is a network-stack-shaped protocol underneath. Worth citing specifically for where it stops: it concludes GPU access is where a fully static binary must give up and accept a dynamic loader ("ICDs will always depend on the dynamic loader... it's futile"). `DESIGN-onebin.md §13.5` (SoLo) is this project's answer to that specific pessimism — a real static musl binary `dlopen`ing a real Mesa/Vulkan ICD with no dynamic loader involved at all. Recorded as independent confirmation of the thesis, and as the reason §13.5 exists.
