...
> **Correction, from actual builds.** The pinned SDL preview currently
> installs with the standard split even under the custom prefix used by
> `tools/build-far2l.sh`: `<prefix>/bin/far2l` and
> `<prefix>/lib/far2l/far2l_sdl.so` (plus the broker and plugins). An older
> custom-prefix build observed in this project was flat, so the builder
> keeps the offline plan stable and detects the installed split directory
> at execution time. Re-verify the layout when changing the far2l pin.
...
- **OpenSSL is explicitly permitted by far2l's LICENSE.txt**: far2l's license
  already includes the standard **OpenSSL Linking Exception** grant at the
  bottom of `LICENSE.txt` ("In addition, as a special exception, the copyright
  holders give permission to link the code of portions of this program with
  the OpenSSL library..."). Statically linking OpenSSL into far2l is 100% legally
  compliant without any workarounds, enabling FTPS and AWS S3 in NetRocks out
  of the box.
