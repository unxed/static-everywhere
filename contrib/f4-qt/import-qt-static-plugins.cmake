...
function(_static_everywhere_import_qt_plugins)
    ...
    # The tree's own QML plugins, restored now that they are legal.
    #
    # This block existed once, failed on `MODULE_LIBRARY may not be
    # linked`, and was removed on the belief Qt's finalizer covers it.
    # Half true: the finalizer imports qml plugins for an executable by
    # scanning that executable's OWN qml sources, so f4-qt-host gets its
    # imports and the eight test executables -- which have no qml files of
    # their own and load everything from linked resources -- get nothing.
    # Run 2026-08-27/night5: 41 x `module "ZoinGallery" is not installed`,
    # from the tests, with the app-side plugin fixed.
    #
    # Two things changed since the first attempt. The backing library is
    # forced STATIC (force-static-qml-backing.cmake), so the plugin is a
    # STATIC_LIBRARY and linking it into executables is legal. And the
    # class name is read from Qt's own QT_PLUGIN_CLASS_NAME property --
    # the same read-not-derive rule as everywhere else here. A MODULE
    # plugin showing up anyway is fatal with its name: it would mean the
    # force-static rewrite stopped covering a target, and silently
    # skipping it is how this bug looked the first time.
    ...
    while(_dirs)
        ...
        foreach(_t IN LISTS _tgts)
            get_target_property(_pcls "${_t}" QT_PLUGIN_CLASS_NAME)
            if(NOT _pcls)
                continue()
            endif()
            get_target_property(_ty "${_t}" TYPE)
            if(_ty STREQUAL "MODULE_LIBRARY")
                message(FATAL_ERROR
                    "static-everywhere: in-tree Qt plugin ${_t} is a MODULE "
                    "library, which cannot be linked into the executables. "
                    "Its backing target escaped the force-static rewrite in "
                    "contrib/f4-qt/force-static-qml-backing.cmake; add it "
                    "there.")
            endif()
            list(APPEND _intree_plugins "${_t}")
            string(APPEND _imports "Q_IMPORT_PLUGIN(${_pcls})\n")
        endforeach()
    endwhile()
    ...
    if(_intree_plugins)
        ...
        while(_dirs)
            ...
            list(APPEND _dirs ${_subs})
            get_property(_tgts DIRECTORY "${_d}" PROPERTY BUILDSYSTEM_TARGETS)
            foreach(_e IN LISTS _tgts)
                get_target_property(_ty "${_e}" TYPE)
                if(NOT _ty STREQUAL "EXECUTABLE")
                    continue()
                endif()
                foreach(_pl IN LISTS _intree_plugins)
                    if(NOT _e STREQUAL _pl)
                        target_link_libraries("${_e}" PRIVATE "${_pl}")
                    endif()
                endforeach()
            endforeach()
        endwhile()
        message(STATUS
            "static-everywhere: registered in-tree Qt plugins in every "
            "executable: ${_intree_plugins}")
    endif()

    set(_gen "${CMAKE_BINARY_DIR}/static_everywhere_qt_plugin_import.cpp")
    file(GENERATE OUTPUT "${_gen}" CONTENT
"// Generated by contrib/f4-qt/import-qt-static-plugins.cmake -- do not edit.
#include <QtPlugin>
#include <QtCore/QCoreApplication>
#include <QtCore/QByteArray>
${_imports}
// ZoinGallery embeds its qmldir at :/ZoinGallery (RESOURCE_PREFIX \"/\"),
// while a default engine searches qrc:/qt/qml. The app and most tests
// call addImportPath(\":/\") themselves; two tests do not, and cannot be
// patched from here. QML_IMPORT_PATH accepts resource paths, is read at
// engine construction, and appending never overrides what a process set
// for itself.
static void se_add_qrc_import_path()
{
    QByteArray p = qgetenv(\"QML_IMPORT_PATH\");
    if (!p.isEmpty())
        p.append(':');
    p.append(\":/\");
    qputenv(\"QML_IMPORT_PATH\", p);
}
Q_COREAPP_STARTUP_FUNCTION(se_add_qrc_import_path)
")
