...
function(_se_konsole_prl_closure archive out_var)
    ...
    # not link items for this consumer and must never cross into a CMake link
    # interface. Qt's component interface already carries these Conan
    # libraries. Drop every :: token, even when a same-named target happens to
    # exist in the package directory: its directory-local existence is not a
    # valid guarantee that the eventual consumer can resolve the interface.
...
function(_se_konsole_import_one archive imports_var libs_var)
    ...
    # Do not append to a same-named variable inherited from the caller's
    # function scope: string(APPEND) would duplicate the complete prefix on
    # every plugin and emit repeated registrations. Build one unit, then
    # append it explicitly to the caller's accumulator.
    set(_import "Q_IMPORT_PLUGIN(${_class})\n")
    set(_new_libs "${archive}")
    list(APPEND _new_libs ${_closure})
    set(${imports_var} "${${imports_var}}${_import}" PARENT_SCOPE)
...
function(_se_konsole_import_static_qt_plugins)
    ...
    # CMake validates target source paths while processing target_link_libraries
    # and before the generation phase. file(GENERATE) is therefore too late:
    # write the registration unit during configure so every consumer can see
    # a real source file during validation.
    file(CONFIGURE OUTPUT "${_generated}" CONTENT
