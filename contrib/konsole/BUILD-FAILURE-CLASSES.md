...
| **C.3. GUI helpers run before `QApplication`** | Static startup reached `KIconTheme::initTheme()` before the application object, then aborted with `QWidget: Must construct a QApplication before a QWidget` while resolving KDE plugin paths | Keep the complete GUI-helper startup set after `QApplication`: apply the exact pinned source overlay, validate the patched source order in the source-only preflight, and re-check it after the idempotent CMake overlay. This guards the initialization-order contract rather than suppressing one abort message | in progress |
