...
| 1.20 | Cached source overlay leaves a dirty checkout that kde-builder refuses to update | Konsole source was modified by the CMake overlay in a restored cache; the next run stopped at `Refusing to stash changes from other branch` | P/C | clean every Git checkout under the cached source root before the updater and on every build exit; regression test covers tracked and untracked overlay residue |
...
| 4.3 | Missing runtime data (ICU data, QPA plugins, KF6 plugins in MODULE form) | P/C | The smoke run on the runner sees every build-time path -- Conan cache (ICU `.dat`, fontconfig's `res/etc`, Qt plugin prefix) and the install tree -- so it passes for binaries broken elsewhere. A second smoke run copies only the portable runtime bundle, hides the Conan cache and build install tree, disables `LD_LIBRARY_PATH`, and invokes the bundle launcher; the launcher and smoke harness supply relocatable `XDG_DATA_DIRS` and `QT_PLUGIN_PATH`, while the executable keeps `$ORIGIN/../lib`. The recipe pins `KDE_INSTALL_PLUGINDIR` and `KDE_INSTALL_QTPLUGINDIR` to relative `lib/plugins`, preventing upstream `${...}/subdir` destinations from escaping the install prefix. The packager leaves every nested install-prefix MODULE out (`bin/kf6/`, `kf6/`, `lib/plugins/`, ...): static Qt cannot load one, and the first green bundle carried 123 MB of them. It keeps relative in-tree `share/` symlinks (breeze-icons is 94% links; dereferencing made 470 MB of 9 MB), replaces absolute or escaping ones by their targets, and drops build-time-only data listed in `runtime-share-exclude.txt`; the packaging and payload checks protect all three. the isolated smoke keeps a scoped `LD_DEBUG=libs,files` trace, and failed-run diagnostics include both smoke logs. Static Qt adds a second failure mode: `QPluginLoader` cannot load a KDE `MODULE` at all, so the KWindowSystem X11 backend is moved -- sources taken from the MODULE target, not a hand list -- into `KF6WindowSystem` with `QT_STATICPLUGIN` and registered with `Q_IMPORT_PLUGIN`; the MODULE, which links `KF6WindowSystem`, becomes an empty placeholder, since keeping its objects duplicated every `X11Plugin` symbol. A configure-only fixture with the upstream two-target shape protects that contract; the built bundle is checked for the plugin class and IID in the executable and for no bundled MODULE claiming the IID. Warnings naming a compiled-in application path fail it. Host fontconfig configuration is explicit Layer 2 data: the launcher uses `/etc/fonts` when present, so a missing config warning is degraded but not a bundle-path failure. Pre-checked from recipes/code: ICU `data_packaging` default `archive` would leave konsole's unchecked `ubidi_*` calls with no data -- now `static`; QPA xcb is imported; KF6 MODULE plugins are bundled |
| 4.4 | A window appears but the static runtime is hollow: Qt resources (`.qrc`) of a STATIC library dropped by the linker, a MODULE backend refused by static Qt | P/C | An AUTORCC object in an archive is referenced by nothing and never extracted, so `:/` is empty: Konsole started with only View/Settings/Help menus, no `default.keytab`, no colour schemes. `link-static-qt-resources.cmake` moves the `.qrc` of every STATIC target in the Konsole tree into an OBJECT library whose objects are a link item of its consumers (the `qt_add_resources` shape); a configure-only fixture protects it. The pixel smoke passes for all of this, so `check-konsole-runtime-log.sh` reads both smoke logs: `Could not find any platform plugin`, `into a statically linked Qt library`, `cannot find .rc file`, `Unable to load translator` fail the run, and KWindowSystem must report loading its static plugin |
...
| **C. The build tree is the install prefix** *(closed)* | `OB0060`: `kde-install/libexec/kf6/kdesu`, plugin dirs compiled into binaries | **Build with a neutral prefix and stage with DESTDIR**: `-DCMAKE_INSTALL_PREFIX=/usr` in `cmake-options`, `DESTDIR=$KDE_INSTALL_DIR` exported by `tools/kde-install-and-reconcile.sh` (kde-builder has no DESTDIR of its own — verified in its source), `CMAKE_PREFIX_PATH` and the smoke/artifact paths moved to `$KDE_INSTALL_DIR/usr`. Then no compiled-in path is ever a build path, for every module, permanently. **Checked before implementing:** kde-builder appends its own `-DCMAKE_INSTALL_PREFIX` from `install-dir` *after* `cmake-options` (`kde_cmake.py:365` vs `:343`), so the prefix must come from `install-dir`; it has no `DESTDIR`; it never creates or writes `install-dir`; and it derives only PATH/LD_LIBRARY_PATH/PKG_CONFIG_PATH from it. KDE's configs are relocatable, so `find_package` resolves into the staging tree -- verified on a two-project probe | **done** |
| **C.1. Toolchain debug paths survive the final link** *(closed)* | After C was fixed, `OB0060` still reported Zig's `crti.S`/`crtn.S`/`start-2.33.S` and `/home/username`; the same audit reported `OB0062 debug info not stripped` | Pass `-Wl,--strip-debug` through every source-built KDE executable, SHARED-library and MODULE link boundary. The folded-config regression parses all three linker flag values and the Zig-wrapper probe validates them; no path-specific waiver can hide a new debug-origin leak | done |
| **C.2. Intentional absolute examples look like build paths** | The pinned Konsole UI used `/home/username` as a placeholder, so the ELF string scanner reported it as `OB0060` even though it was not a build directory | User-facing home-directory examples use the symbolic `~` form. Apply the complete source patch only to the exact commit from `deps.lock`; preflight verifies both the commit and forward applicability before CMake, while CMake applies it idempotently on cached checkouts. No path-specific audit waiver is added | done |
| **C.3. KIconThemes startup callback performs GUI work during `QApplication` construction** | Static startup still aborted with `QWidget: Must construct a QApplication before a QWidget` after moving the explicit `KIconTheme::initTheme()` call; the optional BreezeIcons startup hook runs during application construction | Inspect the pinned framework's CMake/source contract and disable its optional in-process `USE_BreezeIcons` integration for the static graph, while retaining the upstream-required `KIconTheme::initTheme()` before `QApplication`. Preflight asserts the targeted option and both halves of startup order; the CMake hook repeats the source invariant | in progress |
| **C.4. Static Qt is duplicated across a loadable boundary** | `BUILD_SHARED_LIBS=OFF` did not change Konsole's explicit `add_library(konsoleapp SHARED ...)`; the executable constructed `QApplication` in one static Qt instance while `MainWindow` in `libkonsoleapp.so` queried another and aborted with `QWidget: Must construct a QApplication before a QWidget` | Keep the application facade `konsoleapp` `STATIC` whenever Qt/KF6 are static, so the executable owns one Qt global-state instance. The exact pinned source patch, source-only gate and configure-time CMake invariant reject a loadable `konsoleapp` target | in progress |
| **C.5. Source overlays poison the persistent kde-builder cache** | A successful overlay-enabled run saved dirty tracked source files; the next run could not switch a detached checkout to the wanted branch and failed before configuring Konsole | Treat the cached source tree as generated state: restore all Git checkouts before kde-builder starts and clean them again on every exit. The preflight fixture proves tracked edits and untracked generated files are removed for every checkout, so no individual patch or module is special-cased | in progress |
...
The install-prefix generator is implemented as `install-dir:
/opt/static-everywhere` plus `DESTDIR` staging from the install wrapper. The
residual debug-path generator is handled at every loadable link boundary,
rather than by waiving whichever path happens to appear in one run.
