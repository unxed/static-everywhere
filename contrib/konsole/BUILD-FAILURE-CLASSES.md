...
| **C.3. KIconThemes startup callback performs GUI work during `QApplication` construction** (misdiagnosis) | Run 77 blamed the BreezeIcons startup hook for `QWidget: Must construct a QApplication`; run 78 aborted identically with it disabled, from the shared `konsoleapp` duplicating static Qt | `USE_BreezeIcons` is back at its default; breeze is compiled in (KF6BreezeIcons), `qsvg`/`qsvgicon` imported, `KIconEnginePlugin` made a static plugin. Preflight asserts the option is absent and `SKIP_INSTALL_ICONS=ON`; the payload verifier requires the three plugin classes | withdrawn |
